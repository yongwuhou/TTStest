...
*/
    class HttpWebSocket
    {
        ...
        public Action<string> ReceiveDone { get; set; }
        public Action<string> ReceiveError { get; set; }
        ...
        public void GetHtmlUseSocket(string url, string postData = null)
        {
            // this.GetHtmlUseSocket(url, null, postData);
            strurl = url;
            strreferer = null;
            strpostData = postData;
            Thread thread1 = new Thread(GetHtmlUseSocket);
            thread1.IsBackground = true;
            thread1.Start();
        }

        private string strurl;                      //请求绝对地址
        private string strreferer;                  //请求来源地址,可为空
        private string strpostData = null;      //post请求参数. 设置空值为get方式请求

        public void GetHtmlUseSocket()
        {
            App.log.Debug("Url:" + strurl);
            string result = string.Empty;

            try
            {
                MemoryStream ms = this.GetSocketResult(strurl, strreferer, strpostData);
            ...
            {
                result = se.ErrorCode.ToString() + this.ErrorMessageSeparate + se.SocketErrorCode.ToString("G") + this.ErrorMessageSeparate + se.Message;
                //App.log.Debug(string.Format("GetHtmlUseSocket:{0}", result));
                HttpError(result);
                return;
            }
            catch (Exception e)
            ...
            HttpLoginDone(result);
            // App.log.Debug(string.Format("GetHtmlUseSocket:{0}" ,result));
        }

        //触发接收数据错误事件
        void HttpError(string result)
        {
            lock (this)
            {
        ...
        //触发接收数据成功事件
        void HttpLoginDone(string result)
        {
            lock (this)
            {
                if (null != ReceiveDone)
        ...
        private MemoryStream GetHttpResult(Uri uri, string referer, string postData)
        {
            ...
            try
            {
                ...
                App.log.Debug(string.Format("uri.Host:{0}, uri.Port:{1}" ,uri.Host, uri.Port));
                if (HttpSocket.Connected)
                {
                    App.log.Debug("HttpSocket.Connected");
...
";
            //string sendString = "{0} {1} HTTP/1.1\nReferer: {2}\nHost: {3}\n";
            //sendString = string.Format(sendString, string.IsNullOrWhiteSpace(postData) ? "GET" : "POST", uri.PathAndQuery, string.IsNullOrWhiteSpace(referer) ? uri.AbsoluteUri : referer, uri.Host);
           ...
           , string.IsNullOrWhiteSpace(referer) ? uri.AbsoluteUri : referer, uri.Host);
            ...
            else
            {
                ...
                sendString += string.Format("Content-Type:text/html\nContent-Length: {0}\n\n{1}", postData.Length, postData);
            }
            App.log.Debug("sendString:" + sendString);
